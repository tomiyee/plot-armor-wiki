...
- `src/app/[serial]/[schema]/[page]/page.tsx` — wiki page view; resolves serial/schema/page, renders the page name, intro chapter, and all active schema sections with their chapter-filtered `page_section_versions` content via `<ReactMarkdown>`. When `schema.hasFloater` is true, also renders a sticky sidebar with the page image and labeled floater rows. Sections with no content row show "No content yet."
